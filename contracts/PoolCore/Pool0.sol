...
contract Pool0 is Initializable, ERC777Upgradeable {
    ...
    */
    function initialize(string memory name, string memory symbol, address _ERCAddress, address _servicer, address approvedServicer) public initializer {
    ...
    */
    function getContractData() public view returns (address, address, uint256, uint256, uint256) {
        return (servicer, ERCAddress, poolLent, getPoolValueWithInterest(), poolBorrowed);
    }
    ...
    */
    ...
    /**  
    *   @dev Function getAllLoans() returns an array of the loanIndexes all users have taken out
    *   @return array on loanIds of loans
    */
    function getAllLoans() public view returns (uint256[] memory) {
        uint256[] memory loanIds = new uint256[](loans.length);

        for (uint i = 0; i < loans.length; i++) {
            loanIds[i] = loans[i].loanId;
        }
    ...
    */
    function getLoanAccruedInterest(uint256 loanId) public view returns (uint256) {
        ...
        uint256 interestPerSecond = loan.principal.mul(loan.interestRate).div(1000000).div(31622400);
        uint256 interestAccrued = interestPerSecond.mul(secondsSincePayment);
    ...
    *   @param loanId is the id for the loan we're looking up
    */
    function getLoanDetails(uint256 loanId) public view returns (uint256, address, uint256, uint256, uint256, uint256) {
        Loan memory loan = loans[loanId];
        //temp interestAccrued calculation because this is a read function
        uint256 interestAccrued = getLoanAccruedInterest(loanId);
        return (loan.loanId, loan.borrower, loan.interestRate, loan.principal, interestAccrued, loan.timeLastPayment);
    }
    ...
    */
    function mintProportionalPoolTokens(address recepient, uint256 amount) private {
        ...
        uint256 totalValue = getPoolValueWithInterest();
        if (totalValue == 0) {
            super._mint(recepient, amount, "", "");
        } else {
            //Calculate proportional to total value (including interest)
            uint256 new_hc_pool = amount.mul(super.totalSupply()).div(totalValue);
    ...
    */
    function redeem(
        ...
        uint256 tokenPrice = getPoolValueWithInterest().mul(1000000).div(super.totalSupply());
    ...
    *   @param amount The size of the potential loan in (probably usdc).
    *   @param borrower The address of the recipient of the loan
    *   @param maxRate The size of the potential loan in (probably usdc).
    */
    function borrow(uint256 amount, address borrower, uint256 maxRate) public {
        //for v1 only allow ServicerAprovedAddresses to "approve" borrower
        require(isApprovedServicer(msg.sender), "Sender is not approved servicer");

        //check the requested interest rate is still available
        uint256 fixedInterestRate = uint256(getInterestRate(amount));
        require(fixedInterestRate <= maxRate, "interest rate increased");


        //create new Loan
        Loan memory newLoan = Loan(loanCount, borrower, fixedInterestRate, amount, 0, block.timestamp);
        loans.push(newLoan);
        userLoans[borrower].push(loanCount);
        ...
        IERC20Upgradeable(ERCAddress).transfer(borrower, amount);
    ...
    */
    ...
    /**  
    *   @dev Function hasUpgradedFunction returns bool depending on if contract has been upgraded or not
    */
