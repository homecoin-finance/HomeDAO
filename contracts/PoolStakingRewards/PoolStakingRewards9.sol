...
contract PoolStakingRewards9 is Initializable {
    ...
    address airdropContract; // Deprecated

    /* PoolStakingRewards6 Variables */
    mapping(address => bool) isApprovedSubMinter;

    /* PoolStakingRewards7 Variables */
    uint16[] private percent;
    uint private percentStartEpoch;

    /* PoolStakingRewards8 Variables */
    ...
    function fillPercentArray(uint length, uint base, uint _percentStartEpoch) public {
        require(msg.sender == guardianAddress, "PoolStakingRewards: unapproved sender");
        percentStartEpoch = _percentStartEpoch;
        percent = new uint16[](length);
        uint256 acc = 65536;
        for (uint i = 0; i < length; i++) {
            acc *= base;
            acc /= (65536);
            percent[i] = SafeCast.toUint16(acc);
        }
        for (uint i = 0; i < length; i++) {
            epochs.push(0);
        }
        _numberOfEpochs += length;
    }

    function getRewardAtEpoch(uint128 epochId) public view returns (uint) {
        require(epochId <= _numberOfEpochs, "PoolStakingRewards: Epoch out of range");
        if (epochId < percentStartEpoch) {
            return _totalAmountPerEpoch;
        } else {
            uint base = _totalAmountPerEpoch.div(2);
            uint index = epochId - percentStartEpoch;
            uint percentVal = uint(percent[index]).mul(1e18).div(65536);

            return base.mul(percentVal).div(1e18);
        }
    }
    ...
    function _harvest (address wallet, uint128 epochId) internal returns (uint) {
        ...
        return getRewardAtEpoch(epochId)
