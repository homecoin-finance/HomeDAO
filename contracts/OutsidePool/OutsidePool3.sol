...
import "./../RealWorldAsset/RealWorldAsset.sol";

// import "hardhat/console.sol";

contract OutsidePool3 is Initializable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping(address => uint128) private lastEpochIdHarvested; // Deprecated
    uint private numberOfEpochs;

    mapping(address => mapping(address => uint128)) private lastEpochIdHarvestedByPool;
    ...
    function setNewCoinAddress(address coinAddress) public {
        require(msg.sender == guardianAddress, "OutsidePool: unapproved sender");
        newCoinAddress = coinAddress;
    }
    ...
    function massHarvest(address pool, address wallet) external whenNotPaused nonReentrant returns (uint){
        ...
        uint128 startEpoch = lastEpochIdHarvestedByPool[pool][wallet];

        for (uint128 i = startEpoch + 1; i <= epochId; i++) {
            // i = epochId
        ...
        emit MassHarvest(wallet, epochId - startEpoch, totalDistributedValue);

        if (totalDistributedValue > 0) {
            RealWorldAsset(newCoinAddress).transfer(wallet, totalDistributedValue);
        }
    ...
    function harvest (address pool, address wallet, uint128 epochId) external whenNotPaused nonReentrant returns (uint){
        ...
        require (lastEpochIdHarvestedByPool[pool][wallet].add(1) == epochId, "OutsidePool: Harvest in order");
        uint userReward = _harvest(pool, wallet, epochId);
        if (userReward > 0) {
             RealWorldAsset(newCoinAddress).transfer(wallet, userReward);
        }
    ...
    function userLastEpochIdHarvested(address pool) external view returns (uint){
        return lastEpochIdHarvestedByPool[pool][msg.sender];
    }
    ...
    function _harvest(address pool, address wallet, uint128 epochId) internal returns (uint) {
        ...
        lastEpochIdHarvestedByPool[pool][wallet] = epochId;
    ...
    function copyEpochs(address pool, address[] calldata wallets) public {
        require(msg.sender == guardianAddress, "OutsidePool: unapproved sender");
        for (uint128 i = 0; i < wallets.length; i++) {
          address wallet = wallets[i];
          lastEpochIdHarvestedByPool[pool][wallet] = lastEpochIdHarvested[wallet];
        }
    }
}
